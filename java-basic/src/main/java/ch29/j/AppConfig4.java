...
// @Configuration // Srping IoC 컨테이너에게 이 클래스를 직접 알려주기 때문에 이 애노테이션은 필요없다.
...
public class AppConfig4 {
  ...
  // Spring IoC 컨테이너는 이 메서드를 호출한 후 그 리턴 값을 메서드 이름으로 저장한다.
  // 만약 @Bean 애노테이션에 따로 이름을 지정한다면 그 이름으로 리턴 값을 지정한다.
  //
  @Bean
  ...
  // Spring ioC 컨테이너로부터 객체를 주입받고 싶다면 파라미터에 선언하라
  @Bean
  ...
  // Spring ioC 컨테이너를 파라미터로 주입 받을 수 있다.
  @Bean
