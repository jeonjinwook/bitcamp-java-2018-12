...
//      - 의존 객체 주입
//      - 사용할 객체를 직접 만들지 않고 외부에서 주입 받아 사용하는 바익
...
//      - 메서드 호출 코드가 있다면 해당 메서드를 호출한다. 그리고 호출이 끝나면 원래 위치로 이동하여
//          다음 코드를 실행한다.
...
//      - 직접 호출하는 것이 아니라 내부의 의해 호출되는 메서드라는 의미로
...
//      => 기본의 name 필드로 객체 이름을 지정하는 대신에 애노테이션으로 객체 이름을 지정한다.
