...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    //          즉, JVM이 자바 클래스 파일(.class)을 로딩하기 위해 찾는 경로인다.
    //          보통 JVM을 실행할 때 -classpath 옵션이나 -cp 옵션으로 옵션으로 경로를 지정한다.
    ...
    //      => 설정 파일 경로를 지정할때 직접 파일 경로를 지정해야 한다.
    //      => 주의!
    //          설정 파일 경로를 지정할 때 URL 형식으로 지정해야 한다.
    //          URL 형식에서는 파일 시스템을 가리킬 때 다음의 접두어를 붙인다.
    //          file://
    ApplicationContext iocContainer2 = 
        new FileSystemXmlApplicationContext("file:///home/jeonjinwook/git/bitcamp-java-2018-12/java-basic/bin/main/ch29/a/application-context.xml");
