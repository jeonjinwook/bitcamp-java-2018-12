...
//      => ~/server/$톰켓홈
// 2) 자바 웹 프로젝트로 전환
//      => build.gradle 에 'eclipse-wtp' 플로그인과 'war'플러그인 추가한다.
...
//  2) 웹 애플리케이션을 테스트 할 때 사용할 톰캣 배치 폴더
//      - 'Servers' 뷰 선택
//      - New/Server 선택
...
public class ServerApp implements Servlet {
  ...
  public void service(javax.servlet.ServletRequest req, javax.servlet.ServletResponse res)
    ...
    // 톰캐 서버가 파라미터로 넘겨준 값을 원래의 타입으로 변환해서 사용하자.
  ...
  public ServletConfig getServletConfig() {
    // init() 호출될 때 파라미터로 받은 ServletConfig 객체를 그대로 리턴하면 된다.
    return this.config;
  }
