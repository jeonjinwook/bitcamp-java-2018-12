...
// 특정 패키지를 탐색하여 @Componet 애노테이션이 붙은 클래스에 대해 인스턴스를 생성하게 만드는 애노테이션
...
public class AppConfig {
  ...
  public DataSource dataSource() {
    ...
    // stpring IoC 컨테이너에서 주입 받은 env 객체를 사용하여 이 파일의 값을 객체를 보관한다.
  ...
  // Connection의 commit/rollback 을 관리할 트랜잭션 관리자를 생성하는 메서드
  //    => 트랜잭션 매니저의 이름은 "transactionManager" 이어야 한다.
  //        만약 다른 이름을 사용한다면 트랜잭션 매니저를 다룰 때 마다
  //        정확하게 그 이름을 지정해야 하는 번거로움이 발생한다.
  //    => Spring은 트랜잭션 관리자로서 PlatformransactionManager 구현체를 사용한다.
  ...
  public SqlSessionFactory sqlSessionFactory(
    ...
    // 도메인 클래스 별명을 자동으로 생성하도록
    // 도메인 클래스가 들어 있는 패키지를 지정한다.
  ...
  //    => @MapperScan 애노테이션을 사용하면 더 간단히 DAO 구현체를 자동 생성할 수 있따.
  @Bean
