...
public class Controller05_3 {
  ...
  // 1) 요청 파라미터 값을 낱개로 입력받기
  // 테스트:
  //    => http://.../app2/c05_2/h1?no=1&title=ok&writer=kim&viewCount=100
  ...
  // 2) 요청 파라미터 값을 객체로 입력받기
  // 테스트:
  //    => http://.../app2/c05_2/h2?no=1&title=ok&writer=kim&viewCount=100
  ...
  // 3) JSON 형식의 요청 파라미터 값을 통째로 받기
  // 테스트:
  //    => http://.../html/app2/c05_3_h3.html
  ...
  //    => HttpMessageConverter 구현체(예 : MappingJackson2HttpMessageConverter)가ㅏ
  ...
  //    => HttpMessageConverter 구현체(예 : MappingJackson2HttpMessageConverter)가ㅏ
  //        클라이언트가 보낸 데이터를 도메인(예 : Board, Member, Lesson 등)에 담아 준다.
  //    => Json 데이터의 프로퍼티 명과 도메인 객체의 포로퍼티 명이 일치해야 한다.
  //
  // 테스트:
  //    => http://.../html/app2/c05_3_h3.html
  ...
  public Object handler5(@RequestBody Board content) throws Exception {
    ...
    //  => 뿐만 아니라, 도메인 객체를 JSON 문자열로 변활할 때도
    //      해당 형식으로 변환된다.
    // 
