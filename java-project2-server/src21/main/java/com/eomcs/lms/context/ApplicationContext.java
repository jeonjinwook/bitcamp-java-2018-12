...
public class ApplicationContext {
  ...
  public ApplicationContext(Class<?> configClass) throws Exception {
    // IoC 컨테이너와 관련된 설정 정보를 갖고 있는 클래스 정보를 받아서 초기화를 시킨다.
  ...
  private void findClasses(File dir, String packageName) throws Exception {
    ...
    for (File f : files) {
      if (f.isFile()) {
        ...
        // => 클래스 정보를 분석하여 중첩 클래스 이거나 인터페이스, Enum이면 무시한다.
  ...
  private Object[] getParameterValues(Class<?>[] paramTypes) {
    // 파라미터에 타입에 해당하는 객체를 빈컨테이너에서 찾아 배열을 만들어 리턴한다.
    ...
    if (values.size() == paramTypes.length)
      // 파라미터의 타입 배열에 지정된 값을 모두 찾았으면 배열로 리턴한다.
  ...
  private Object callFactoryMethod(
    ...
    // 파라미터 값을 담을 배열을 준비한다.
    //      ex) {new BoardDao(), new MemberDao(), new TrascationManager()}
    ...
    for (int i = 0 ; i < paramTypes.length; i++) {
      ...
      if (otherFactoryMethod == null) {
        // 4) 만약 해당 파리미터 타입의 값을 만들어주는 다른 팩토리 메서드가 없다면
