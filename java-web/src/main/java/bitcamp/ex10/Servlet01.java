...
Set-Cookie: name2=홍길동 <--- URL 인코딩 안할때
Set-Cookie: name3=%ED%99%8D%EA%B8%B8%EB%8F%99 <---URL 인코딩한후
Content-Type: text/plain;charset=UTF-8
Content-Length: 35
Date: Wed, 03 Apr 2019 01:03:57 GMT
     */
    // 쿠키 만들기
    //  => 이름과 값으로 생성한다.
    //  => 쿠키의 유효기간을 설정하지 않으면 웹브라우저가 종료될 때 까지 유지된다.
    //      웹 브라우저를 종료하면 웹 서버로 부터 받은 유효기간이 지정되지 않은 쿠키를 모두 삭제된다.
    //  => 쿠키의 사용범위를 지정하지 않으면 현재 경로에 한정한다.
    //      쿠키를 보낼 때에 URL이 /ex10/s1 이라면,
    //      웹 브라우저는 /ex10/* 경로를 요청할 때만 웹서버에게 쿠키를 보낸다.
    Cookie c1 = new Cookie("name", "hong");
    
    //  => 값은 반드시 문자열이어야 한다.
    //      만약 문자열이 아닌 값을 보내려면 Base64와 같은 인코딩기법을 이용하여
